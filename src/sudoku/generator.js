...
function search(values) {
  ...
  if (all([...squares].map((s) => values.get(s).size === 1))) return values;
  ...
  for (let s of shuffle(squares)) {
    ...
    if (ds.length >= n && new Set(ds).size >= 8) {
      return [...squares].map((s) =>
        values.get(s).size === 1 ? [...values.get(s)][0] : "0"
      );
    }
  }
 ...
 * try another square.
 */
function createPuzzle(solution, n) {
  let puzzle = [];
  let indices = {};
...
export default {
  getGame: (difficulty = level.medium) => {
    ...
    return {
      puzzle: createPuzzle(solution, 81 - difficulty),
      solution: solution,
    };
  },
};
